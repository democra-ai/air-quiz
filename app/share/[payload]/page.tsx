...
import { inferOccupation, inferOccupationFromAnswers, unpackDimAvg, unpackCoreAnswers } from '@/lib/occupation_inference';
...
export default async function SharePage({ params, searchParams }: Props) {
  ...
  // Per-answer occupation inference. Prefer the new 16-answer path (v2.1+);
  // fall back to the 4-dim-average path for older share URLs.
  ...
  const guesses = coreAnswers
    ? inferOccupationFromAnswers(coreAnswers, 3)
    : (dimAvg ? inferOccupation(dimAvg, 3) : []);
  const inferredJobs = guesses.map((g) => ({
    id: g.id,
